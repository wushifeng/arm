...
ioctl是设备驱动程序中对设备的I/O通道进行管理的函数int ioctl(int fd_cam,int cmd,...)?
成功返回0，出错返回-1
其中fd_cam--就是用户程序打开设备使用open函数返回的文件标识符
...
*/
    int ret = ioctl(fd_cam, VIDIOC_QUERYCAP, &cap);
	if (ret < 0) {
...
*/
	...
	ret=ioctl(fd_cam, VIDIOC_ENUM_FMT, &fmtdesc);
	while (ret != 0)
    {
        fmtdesc.index++;
		ret=ioctl(fd_cam, VIDIOC_ENUM_FMT, &fmtdesc);
...
*/
    ...
    int ret = ioctl(fd_cam, VIDIOC_S_FMT, &fmt);
	if (ret < 0) {
...
*/
    ...
    int ret = ioctl(fd_cam , VIDIOC_REQBUFS, &reqbuf);
    if(ret < 0) {
...
*/
    ...
    for (i = 0; i < reqbuf.count; i++)
    {
        ...
        ret = ioctl(fd_cam , VIDIOC_QUERYBUF, &buf);//buf取得内存缓冲区的信息
        if(ret < 0) {
...
/*
#include <sys/mman.h>
void *mmap(void *start, size_t length, int prot, int flags,int fd_cam, off_t offset);
...
fd_cam：有效的文件描述词。一般是由open()函数返回，其值也可以设置为-1，此时需要指定flags参数中的MAP_ANON,表明进行的是匿名映射
offset：被映射对象内容的起点


成功执行时，mmap()返回被映射区的指针，munmap()返回0。失败时，mmap()返回MAP_FAILED[其值为(void *)-1]，munmap返回-1
*/
        framebuf[i].start = (char *) mmap(0, buf.length, PROT_READ|PROT_WRITE, MAP_SHARED, fd_cam, buf.m.offset);
...
*/
    ret = ioctl(fd_cam, VIDIOC_DQBUF, &buf);//VIDIOC_DQBUF命令结果, 使从队列删除的缓冲帧信息传给了此buf
    if (ret < 0) {
